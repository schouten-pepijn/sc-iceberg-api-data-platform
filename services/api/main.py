...
@app.get(
    "/forecast/accuracy",
    response_model=list[ForecastAccuracyResponse],
)
def get_forecast_accuracy(
    ...
    """Return forecast-vs-observed accuracy rows with optional date/location filters."""
    if start_date is not None and end_date is not None and start_date > end_date:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="start_date must be less than or equal to end_date",
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="fact_forecast_accuracy table does not exist. Materialize the accuracy fact first.",
        ) from exc
    ...
    df = df.sort_values(["location_id", "target_timestamp"]).head(limit)
